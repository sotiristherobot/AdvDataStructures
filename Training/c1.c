...
typedef int item_t;
typedef struct st_t { item_t item; struct st_t *next; } stack_k;

typedef stack_k node_t;
...
int stack_empty(stack_k *st)
{   return( st->next == NULL );
}

void push( item_t x, stack_k  *st)
{   stack_k *tmp;
    tmp = get_node();
    tmp->item = x;
    tmp->next = st->next;
...
item_t pop(stack_k *st)
{   stack_k  *tmp; item_t tmp_item;
    tmp = st->next;
    st->next = tmp->next;
    tmp_item = tmp->item;
    return_node( tmp );
...
item_t top_element(stack_k *st)
{   return( st->next->item );
}

void remove_stack(stack_k *st)
{   stack_k *tmp;
    do
    {  tmp = st->next; 
       return_node(st);
